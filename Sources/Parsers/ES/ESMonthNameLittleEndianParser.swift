...
public class ESMonthNameLittleEndianParser: Parser {
    ...
    override public func extract(text: String, ref: ChronoDate, match: NSTextCheckingResult, opt: [OptionType: Int]) throws -> ParsedResult? {
        ...
        let month = ES_MONTH_OFFSET[try match.string(from: text, atRangeIndex: monthNameGroup).lowercased()]!
        ...
        if match.isNotEmpty(atRangeIndex: weekdayGroup) {
            let weekday = ES_WEEKDAY_OFFSET[try match.string(from: text, atRangeIndex: weekdayGroup).lowercased()]
