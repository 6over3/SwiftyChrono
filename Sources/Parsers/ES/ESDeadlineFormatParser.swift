...
public class ESDeadlineFormatParser: Parser {
    ...
    override public func extract(text: String, ref: ChronoDate, match: NSTextCheckingResult, opt: [OptionType: Int]) throws -> ParsedResult? {
        ...
        var date = ref
        if try NSRegularExpression.isMatch(forPattern: "d[ií]a", in: number4) {
        ...
        if try NSRegularExpression.isMatch(forPattern: "hora", in: number4) {
            date = number != HALF ? try date.added(number, .hour) : try date.added(30, .minute)
        } else if try NSRegularExpression.isMatch(forPattern: "minuto", in: number4) {
