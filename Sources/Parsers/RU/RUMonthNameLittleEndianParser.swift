...
private func parseDay(from match: NSTextCheckingResult, in text: String, fullGroup: Int, numGroup: Int) throws -> Int? {
    if match.isNotEmpty(atRangeIndex: numGroup) {
        return Int(try match.string(from: text, atRangeIndex: numGroup))
    }
    if match.isNotEmpty(atRangeIndex: fullGroup) {
        return try ruOrdinalValue(of: try match.string(from: text, atRangeIndex: fullGroup))
    }
    return nil
}
...
public class RUMonthNameLittleEndianParser: Parser {
    ...
    override public func extract(text: String, ref: ChronoDate, match: NSTextCheckingResult, opt: [OptionType: Int]) throws -> ParsedResult? {
        ...
        guard let day = try parseDay(from: match, in: text, fullGroup: dateGroup, numGroup: dateNumGroup) else {
            return nil
        }
        ...
        if match.isNotEmpty(atRangeIndex: weekdayGroup) {
            let weekday = RU_WEEKDAY_OFFSET[try match.string(from: text, atRangeIndex: weekdayGroup).lowercased()]
            result.start.assign(.weekday, value: weekday)
        }
        
        // Text can be "range" value. Such as "12 - 13 January 2012"
        if let dayTo = try parseDay(from: match, in: text, fullGroup: dateToGroup, numGroup: dateToNumGroup) {
