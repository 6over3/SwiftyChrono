...
public class FRTimeExpressionParser: Parser {
    ...
    override public func extract(text: String, ref: ChronoDate, match: NSTextCheckingResult, opt: [OptionType: Int]) throws -> ParsedResult? {
        ...
        hour = Int(try match.string(from: secondText, atRangeIndex: hourGroup).replacingOccurrences(of: "h", with: ""))!
        ...
        if match.isNotEmpty(atRangeIndex: amPmHourGroup) {
            ...
            let ampm = String(try match.string(from: secondText, atRangeIndex: amPmHourGroup).substring(from: 0, to: 1)).lowercased()
            if ampm == "a" {
                meridiem = 0
                if hour == 12 {
                    hour = 0
