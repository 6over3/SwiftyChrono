...
public class DEMonthNameLittleEndianParser: Parser {
    ...
    override public func extract(text: String, ref: ChronoDate, match: NSTextCheckingResult, opt: [OptionType: Int]) throws -> ParsedResult? {
        ...
        let month = DE_MONTH_OFFSET[try match.string(from: text, atRangeIndex: monthNameGroup).lowercased()]!
        
        let day = match.isNotEmpty(atRangeIndex: dateNumGroup) ?
            Int(try match.string(from: text, atRangeIndex: dateNumGroup))! :
            DE_ORDINAL_WORDS[try match.string(from: text, atRangeIndex: dateGroup).trimmed().lowercased()]!
        ...
        if match.isNotEmpty(atRangeIndex: yearGroup) {
            ...
            if match.isNotEmpty(atRangeIndex: yearBeGroup) {
                ...
                if try NSRegularExpression.isMatch(forPattern: "v", in: yearBe) {
                    // Before Christ
                    year = -year
                }
            } else if year < 10 {
        ...
        if match.isNotEmpty(atRangeIndex: weekdayGroup) {
            let weekday = DE_WEEKDAY_OFFSET[try match.string(from: text, atRangeIndex: weekdayGroup).lowercased()]
        ...
        if match.isNotEmpty(atRangeIndex: dateToGroup) {
            let endDate = match.isNotEmpty(atRangeIndex: dateToNumGroup) ?
                ...
                DE_ORDINAL_WORDS[try match.string(from: text, atRangeIndex: dateToGroup).trimmed().replacingOccurrences(of: "-", with: " ").lowercased()]
