...
class ExtractTimezoneAbbrRefiner: Refiner {
    override public func refine(text: String, results: [ParsedResult], opt: [OptionType: Int]) throws -> [ParsedResult] {
        ...
        let availableTagUnits: Set<TagUnit> = [.enTimeExpressionParser, .frTimeExpressionParser, .zhHantTimeExpressionParser, .zhHansTimeExpressionParser]
        ...
        while i < resultsLength {
            ...
            {
                let timezoneAbbr = (match.isNotEmpty(atRangeIndex: 1) ? try match.string(from: substring, atRangeIndex: 1) : "").uppercased()
